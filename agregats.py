...
from pyspark.sql.functions import col, mean, month, to_date, udf, year
...
spark = SparkSession.builder.config(
    "spark.driver.extraClassPath", "./mysql-connector-java-8.0.29.jar"
).getOrCreate()
...
# %%
all_stations = all_stations.na.drop(how="any", subset=["TMP"]).filter(
    ~all_stations.TMP.contains("+9999")
)
...
# %%
# Par année
mean_tmp_by_year = all_stations.groupBy([year("DATE").alias("year")]).agg(
...
# %%
# Par mois
mean_tmp_by_month = all_stations.groupBy(
    [year("DATE").alias("year"), month("DATE").alias("month")]
...
# %%
# Par jour
mean_tmp_by_day = all_stations.groupBy(to_date("DATE").cast("date").alias("date")).agg(
    mean("temperature").alias("mean_tmp")
)
...
# %%
# Par saisons
mean_tmp_by_season = all_stations.groupBy([year("DATE").alias("year"), "season"]).agg(
    mean("temperature").alias("mean_tmp")
)
...
# %%
# Par année
min_max_tmp_by_year = all_stations.groupBy([year("DATE").alias("year")]).agg(
    F.min("temperature").alias("min_tmp"), F.max("temperature").alias("max_tmp")
)
...
# %%
# Par mois
min_max_tmp_by_month = all_stations.groupBy(
    [year("DATE").alias("year"), month("DATE").alias("month")]
).agg(F.min("temperature").alias("min_tmp"), F.max("temperature").alias("max_tmp"))
min_max_tmp_by_month = min_max_tmp_by_month.sort("year", "month")
min_max_tmp_by_month.head(10)

# %%
# Par jour
min_max_tmp_by_day = all_stations.groupBy(
    to_date("DATE").cast("date").alias("date")
).agg(F.min("temperature").alias("min_tmp"), F.max("temperature").alias("max_tmp"))
...
# %%
# Par saisons
min_max_tmp_by_season = all_stations.groupBy(
    [year("DATE").alias("year"), "season"]
).agg(F.min("temperature").alias("min_tmp"), F.max("temperature").alias("max_tmp"))
...
min_max_tmp_by_month_and_city = all_stations.groupBy(
    [
        col("NAME").alias("ville"),
        col("STATION"),
        year("DATE").alias("year"),
        month("DATE").alias("month"),
    ]
).agg(F.min("temperature").alias("min_tmp"), F.max("temperature").alias("max_tmp"))
min_max_tmp_by_month_and_city = min_max_tmp_by_month_and_city.sort(
    "ville", "STATION", "year", "month"
)
...
# %%
mean_tmp_elevation_by_year = all_stations.groupBy(
    [year("DATE").alias("year"), "STATION"]
).agg(mean("temperature").alias("mean_tmp"), mean("ELEVATION").alias("mean_elevation"))
...
# %%
g = sns.relplot(
    x="mean_tmp", y="mean_elevation", hue="year", data=df_pandas, palette="magma"
)
